...
C-----------------------------------------------------------------------
C
      netcdf_file='pom2k6549.nc'  ! netCDF output file
...
C-----------------------------------------------------------------------
      call CPU_TIME(start)
!$acc data create(elf,dtef,l,uaf,vaf,zflux,uf,vf,aam2d,fluxua)
!$acc data create(fluxva,ady2d,adx2d,advua,advva,drx2d,dry2d,advx,a,c) 
!$acc data create(ee,drhox,drhoy,tps,gg,vtf,utf,wusurf,advy,egf)
!$acc data copyin(ele,elw,uabw,uabe,vabs,ua,va,el,q2b,q2)           
!$acc data copyin(q2lb,q2l,kq,sclim,tclim,swrad,wtsurf,wssurf,dx,dy)
!$acc data copyin(tbe,sbe,tbw,sbw,sbn,tbn,tbs,sbs,kh,cbc)
!$acc data copyin(dzz,w,utb,vtb,egb,etb,wubot,wvbot,ub,vb)
!$acc data copyin(rmean,els,eln,etf,vfluxb,vabn,vfluxf,art,h,wvsurf)
!$acc data copyin(cor,e_atmos,dum,dvm,arv,aru,uab,vab,u,v)
!$acc data copyin(t,s,rho,aam,km,z,fsm,dz,dt,tb)
!$acc data copyin(sb,et,elb,d,zz,tsurf,ssurf) 
...
!$acc parallel loop gang vector collapse(2) default(present)   
        do j=1,jm
          do i=1,im
            egf(i,j)=el(i,j)*ispi
            utf(i+1,j)=ua(i+1,j)*(d(i+1,j)+d(i,j))*isp2i
            vtf(i,j+1)=va(i,j+1)*(d(i,j+1)+d(i,j))*isp2i
          end do
        end do
!$acc end parallel
...
!$acc update host(uab,vab,u,v,w,t,s,rho,aam,km,z) if(.true.)
!$acc update host(fsm,dz,dt,tb,sb,dx,dy,et,elb,d,zz) if(.true.)
...
C
      integer i,j,k
      logical flag
!$acc parallel loop gang vector collapse(3) default(present) if(flag)     
      do j=1,jm
        do i=1,im
          do k=1,kb
...
!$acc parallel loop gang vector collapse(2) default(present) if(flag)
      do j=2,jmm1
        do i=2,imm1
...
C
      subroutine dens(si,ti,rhoo,flag)
C **********************************************************************
C *                                                                    *
...
!$acc data create(gh,boygr,stf,prod,kn,l0)
!$acc parallel loop gang vector collapse(2) default(present)
      do j=1,jm
        do i=1,im
